...
export class AbstractLoggingDestination {
  ...
  // gets the minimum level for a given tag that we will log at
  ...
  public setDefaultFilter = (level :logging.Level) => {
    ...
    // this could theoretically change anything we have a listener for so we
    // save every possible level
...
function formatMessage(l:logging.Message) : string {
  return logging.Level[l.level] + ' [' + dateToString_(l.timestamp) + '] ' + l.message;
}
...
// we should handle unbinding the listener if we get disconnected, that can be
// taken care of later
