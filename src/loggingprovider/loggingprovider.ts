...
class BufferedTransport extends LoggingTransport {
  constructor() {
    super({'*': 'E'});
  }
...
var logTransports :{[name :string] :LoggingTransport} = {
  console: new ConsoleTransport(),
  buffered: new BufferedTransport()
}
...
export function doRealLog(level:string, tag:string, msg:string)
  ...
  for (var i in logTransports) {
    logTransports[i].log(level, tag, message);
  }
}
...
export class LoggingController implements logging.Controller  {
  ...
  public setConsoleFilter = (args: string[]) : void => {
    logTransports['console'].setFilter(args);
  }
  ...
  public setBufferedLogFilter = (args: string[]) : void => {
    logTransports['buffered'].setFilter(args);
  }
}
